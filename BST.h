...
class BST
{
 public:
  ...
  bool search(const DataType & item, DataType&) const; 
  /*------------------------------------------------------------------------
...
//modified to return the object if found through reference parameter
template <typename DataType>
bool BST<DataType>::search(const DataType & item, DataType & other) const
{
   ...
   while (!found && locptr != 0)
   {
	  ...
	  {
		  found = true;
		  other = locptr->data;		//change the value of the reference parameter only if found
	  }
   }
